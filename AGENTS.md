...
- Serverless batch sizes and running-job stale detection are controlled with `BACKEND_REVIEW_BATCH_SIZE` (default `5`), `BACKEND_IMPORT_BATCH_SIZE` (default `3`), and `BACKEND_RUNNING_STALE_TIMEOUT_SECONDS` (default `300`); queued jobs still use the separate queued timeout in the backend.
