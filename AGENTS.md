...
- `docker compose --profile prod up --build` — run the local prod-like stack (app served via Firebase Hosting emulator from `src/frontend/dist`; still emulator-backed for Firebase services, so it does not validate remote Firebase resource existence).
...
- VS Code launch configs: `Stack: Start (Docker Compose - dev)` and `Stack: Start (Docker Compose - prod)` in `.vscode/launch.json`; stop the running stack via VS Code's Run and Debug stop control.
...
- `Firebase: Deploy` is zero-config for local use: it reads the default project from `.firebaserc`, triggers login when needed, does not prompt for project id, and auto-creates the default Firestore database `(default)` when missing (location defaults to `FIREBASE_REGION` unless `FIRESTORE_DATABASE_LOCATION` is set).
- `Firebase: Clear Project` is zero-config for local use: it reads project/region defaults from `.firebaserc`, confirms interactively (unless `--force`), clears functions/firestore/rtdb/hosting, deletes the default Firestore database `(default)`, deletes Cloud Storage buckets in the project after clearing their objects, and can prompt for `gcloud auth application-default login` when local Storage cleanup lacks Google ADC.
...
- Devcontainer Compose launches via `scripts/docker-compose.sh` suppress Docker Compose's attached shortcut menu (`COMPOSE_MENU=false` by default inside `/.dockerenv`) because host-only actions like "View in Docker Desktop" cannot be opened reliably from the devcontainer terminal; override `COMPOSE_MENU` explicitly if you need that menu.
- Devcontainer startup disables npm update-notifier noise (`NPM_CONFIG_UPDATE_NOTIFIER=false`) for cleaner rebuild logs.
- Devcontainer frontend dependency reinstalls run through `scripts/npm-ci-frontend.sh` so npm unpack/install is more resilient against intermittent `ENOENT`/tarball failures and incomplete `node_modules` trees.
...
- Firebase deploy workflow lives in `.github/workflows/firebase-deploy.yml`, is manual-only via `workflow_dispatch`, and requires `FIREBASE_SERVICE_ACCOUNT` secret.
...
- `scripts/firebase-deploy.sh` also preflights production Firebase web config and fails fast when frontend auth config is incomplete (`VITE_FIREBASE_API_KEY`, `VITE_FIREBASE_APP_ID`, `VITE_FIREBASE_PROJECT_ID`), with best-effort auto-resolution from Firebase Web App SDK config via `firebase apps:sdkconfig`; that same SDK config now seeds managed Functions `GCS_BUCKET` from `storageBucket` when available so artifact uploads target the real project bucket.
- `scripts/firebase-deploy.sh` also preflights Firebase Storage itself: it verifies/discovers the real project bucket via the Storage API, writes managed `GCS_BUCKET` when exactly one/project-matching bucket is found, auto-creates the Firebase-configured default bucket when it is missing (location defaults to `FIREBASE_REGION`, override with `FIREBASE_STORAGE_BUCKET_LOCATION`), falls back to a normal project-owned artifact bucket name when the Firebase-style bucket name is unavailable due to global domain ownership, and reuses that existing fallback bucket on later deploys even if Firebase Web SDK config still reports the unusable default bucket name.
...
- `scripts/firebase-deploy.sh` also preflights the default Cloud Firestore database and auto-creates `(default)` when it is missing from the target project; first-create location defaults to `FIREBASE_REGION` and can be overridden via `FIRESTORE_DATABASE_LOCATION`.
- `scripts/firebase-deploy.sh` also handles Firebase's temporary cooldown after deleting `(default)` by waiting and retrying database recreation up to `FIRESTORE_DATABASE_CREATE_MAX_WAIT_SECONDS` (default `900`).
- Worker memory for the deployed Pub/Sub import worker is configured through `BACKEND_WORKER_MEMORY_MB` (supported values: `128`, `256`, `512`, `1024`, `2048`, `4096`, `8192`, `16384`, `32768`; default managed deploy value `512` MiB via `.github/firebase-deploy.defaults.json`). Raise it explicitly when production pressure justifies it; the deploy preflight prints the effective worker memory.
...
- `scripts/firebase-deploy.sh` now prints a manual Firebase Auth setup overview during deploy (sign-in method, email template branding, custom sender domain, authorized domains). `FIREBASE_AUTH_EMAIL_APP_NAME` (default `Skybridge`) is used as the friendly name hint in that overview only. Firebase Console currently requires Google sign-in provider enabled before "Public-facing name" can be edited in Auth templates, and custom sender domains for Auth emails must still be configured manually in Firebase Console (`Authentication -> Templates -> Email address sign-in -> Customize domain`) with the DNS records Firebase provides.
...
- `scripts/firebase-deploy.sh` installs frontend dependencies via `scripts/npm-ci-frontend.sh` (validated npm install, cache isolation, nested first attempt with hoisted retry fallback) to reduce intermittent install failures.
...
- Serverless review/import orchestration is resumable and batch-based in `src/backend/service.py` and `src/backend/lambda_handlers.py`: workers checkpoint progress in the job record (`heartbeat_at`, `phase_cursor`, `phase_total`), process only a small batch per invocation, and re-enqueue themselves until completion so the flow remains reliable under low-memory Cloud Functions limits.
- Review is metadata-only in the serverless path to reduce peak memory: it builds `review-flights.json` from CloudAhoy metadata without generating export files, while import generates and uploads the actual CloudAhoy export artifacts flight-by-flight.
- Serverless batch sizes and running-job stale detection are controlled with `BACKEND_REVIEW_BATCH_SIZE` (default `1`), `BACKEND_IMPORT_BATCH_SIZE` (default `1`), `BACKEND_RUNNING_STALE_TIMEOUT_SECONDS` (default `210`, aligned just above the frontend delayed-heartbeat warning), and `BACKEND_JOB_CREDENTIAL_TTL` (default `21600` seconds for encrypted job-scoped import/retry credentials); queued jobs still use the separate queued timeout in the backend.
- Review artifacts are split by purpose: `review.json` is the slim resumable import manifest, while `review-flights.json` stores the full Review UI rows so Firestore job documents stay lightweight.
- Firebase Storage job-artifact cleanup is always-on in the backend: clearing a run or expiring it via retention deletes its remote artifact prefix, and the daily cleanup sweep removes orphaned prefixes left behind by older runs.
...
- Firebase deploy workflow lives in `.github/workflows/firebase-deploy.yml`, is manual-only via `workflow_dispatch`, and requires `FIREBASE_SERVICE_ACCOUNT` and `BACKEND_ENCRYPTION_KEY` secrets. `FIREBASE_WEB_APP_ID` and `FIREBASE_APP_CHECK_SITE_KEY` are optional. Shared deploy flow generates temporary production env files from `.github/firebase-deploy.defaults.json`; do not rely on manual Firebase console env configuration.
