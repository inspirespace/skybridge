...
- Serverless batch sizes and running-job stale detection are controlled with `BACKEND_REVIEW_BATCH_SIZE` (default `1`), `BACKEND_IMPORT_BATCH_SIZE` (default `1`), `BACKEND_RUNNING_STALE_TIMEOUT_SECONDS` (default `120`), and `BACKEND_JOB_CREDENTIAL_TTL` (default `21600` seconds for encrypted job-scoped import/retry credentials); queued jobs still use the separate queued timeout in the backend.
