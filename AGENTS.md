...
- Worker memory for the deployed Pub/Sub import worker is configured through `BACKEND_WORKER_MEMORY_MB` (supported values: `128`, `256`, `512`, `1024`, `2048`, `4096`, `8192`, `16384`, `32768`; default managed deploy value `256` MiB via `.github/firebase-deploy.defaults.json`). Raise it explicitly when production pressure justifies it; the deploy preflight prints the effective worker memory.
...
- Serverless batch sizes and running-job stale detection are controlled with `BACKEND_REVIEW_BATCH_SIZE` (default `1`), `BACKEND_IMPORT_BATCH_SIZE` (default `1`), `BACKEND_RUNNING_STALE_TIMEOUT_SECONDS` (default `600`), and `BACKEND_JOB_CREDENTIAL_TTL` (default `21600` seconds for encrypted job-scoped import/retry credentials); queued jobs still use the separate queued timeout in the backend.
