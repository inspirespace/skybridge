...
- `scripts/firebase-deploy.sh` now also preflights Firebase Auth sign-in config for passwordless email-link mode (`signIn.email.enabled=true` and `signIn.email.passwordRequired=false`) when `FIREBASE_REQUIRE_EMAIL_LINK_SIGNIN=1` (default). Auto-enable defaults to ON (`FIREBASE_AUTO_ENABLE_EMAIL_LINK_SIGNIN=1` when unset) and patches this config automatically during deploy preflight; set `FIREBASE_AUTO_ENABLE_EMAIL_LINK_SIGNIN=0` to disable. Token resolution uses ADC (`GOOGLE_APPLICATION_CREDENTIALS`) and falls back to Firebase CLI login token cache.
- `scripts/firebase-deploy.sh` also aligns Firebase Auth email branding by setting the project display name (used as `%APP_NAME%` fallback in auth emails) from `FIREBASE_AUTH_EMAIL_APP_NAME` (default `Skybridge`).
- `scripts/firebase-deploy.sh` also applies Firebase Auth email-template branding fallback (default ON via `FIREBASE_AUTO_PATCH_EMAIL_TEMPLATE_BRANDING=1`) so email-link copy remains branded even when `%APP_NAME%` resolves to project/site identifiers.
