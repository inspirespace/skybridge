...
def user_id_from_request(authorization: Optional[str], _x_user_id: Optional[str] = None) -> str:
...
def user_id_from_event(event: dict[str, Any]) -> str:
    ...
    return user_id_from_request(authorization)
...
def _verify_token(token: str) -> dict[str, Any]:
    ...
    if not issuer:
        raise HTTPException(
            status_code=500,
            detail="Firebase project id could not be resolved for token verification.",
        )
