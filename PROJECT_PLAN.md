...
- Milestone 3: Dev workflow (in progress).
  ...
  - Status:
    - Backend flow: review/import runs via API clients, artifacts written to `data/backend/jobs`, UI polls job status.
    - Layout/structure: footer pinned within the flex page; accordions tightened and collapse without blank rows; status column placed first for mobile scanning.
    - Review/import UI: progress cards styled with success tint; import report condensed into a single Import results summary card; conclusion spacing refined.
    - Tables & badges: status pills standardized and prevent wrapping; tables horizontally scroll on small screens; review status pill rendering fixed.
